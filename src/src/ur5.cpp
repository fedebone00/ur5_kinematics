...
Eigen::Matrix3d eul2rotm(Eigen::Vector3d rpy) {
  ...
  double g = rpy(0);
  Eigen::Matrix3d tmp;
  tmp << cos(a) * cos(b),                           cos(a) * sin(b) * sin(g) - sin(a) * cos(g),     cos(a) * sin(b) * cos(g) + sin(a) * sin(g),
...
Eigen::Vector3d quat2eul(Eigen::Vector4d quat) {
  ...
  Eigen::Vector3d tmp;
  tmp << std::atan2(sinr_cosp, cosr_cosp),
...
const Eigen::Vector<double, 6> UR5::best_angles(const Eigen::Vector<double, 6> &actual, const Eigen::Matrix<double, 8, 6> &possible) {
  ...
  for (int i = 0; i < possible.rows(); ++i) {
    double diff = 0;
    if (!possible.row(i).hasNaN() && possible.row(i)(1) < 0 && possible.row(i)(1) > -M_PI /*&& possible.row(i)(4) < 0*/ && possible.row(i)(3) < 0) {
